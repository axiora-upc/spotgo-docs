...
* **Backend (RESTful Web Services):** Se alojará en la plataforma PaaS **Railway** mediante despliegue continuo desde GitHub. Cada PR fusionado en `main` activará un pipeline de CI/CD que compilará, probará y desplegará automáticamente el backend en producción, asegurando una entrega rápida y confiable de las nuevas funcionalidades.
* **Base de Datos en la Nube:** Se utilizará una instancia gestionada de PostgreSQL en **Neon**, enlazada de forma segura mediante variables de entorno (Environment Variables) al backend en Railway.
